...
func (executor *WaitExecutor) Execute(id uint32, message Message, incoming chan Message, terminate chan bool) error {
...
func TestTransactionEngine_WaitExecutor(t *testing.T) {
	engine, err := NewTransactionEngine(1 << 20)
	if err != nil {
	...
	task1, _ := NewJsonMessage(testMessage)
	task1.SetUInt(KeyTaskID, id1)

	task2, _ := NewJsonMessage(testMessage)
...
func (executor *TimeoutExecutor) Execute(id uint32, message Message, incoming chan Message, terminate chan bool) error {
	taskID, _ := message.GetUInt(KeyTaskID)
...
func TestTransactionEngine_SessionTimeout(t *testing.T) {
	...
	for taskID := 0; taskID < taskCount; taskID++ {
		msg, _ := NewJsonMessage(testMessage)
		msg.SetUInt(KeyTaskID, uint32(taskID))
...
func (executor *NopeExecutor) Execute(id uint32, message Message, incoming chan Message, terminate chan bool) error {
	taskID, _ := message.GetUInt(KeyTaskID)
	resultChan <- taskID
	return nil
}
...
func (executor *OneStepExecutor) Execute(id uint32, message Message, incoming chan Message, terminate chan bool) error {
	taskID, _ := message.GetUInt(KeyTaskID)
	var timer = time.NewTimer(5 * time.Second)
	select {
