...
import (
	"fmt"
	"github.com/mars-projects/mars/common/wire/cache"
	"github.com/mars-projects/mars/common/wire/cache/memory"
	"math/rand"
	"testing"
	"time"
...
func getStore(_ *testing.T) cache.AdapterCache {
...
func TestStore_CollectNotExpire(t *testing.T) {
	...
	if v := s.Get("0", false); v != "0" {
		t.Error("redis store get failed")
	}
}

func TestNewCacheStore(t *testing.T) {
	type args struct {
		store      cache.AdapterCache
		expiration int
	}
	tests := []struct {
		name string
